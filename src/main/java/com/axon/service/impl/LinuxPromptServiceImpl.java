...
/**
 * Implementation of {@link PromptService} specifically designed for the Linux Operating System.
 * <p>
 * This class encapsulates the "Prompt Engineering" strategies required to generate high-quality
 * Linux command-line curricula. It is responsible for:
 * <ul>
 *   <li>Defining the scope of specific Linux modules (e.g., File manipulation vs. Process management).</li>
 *   <li>Enforcing strict JSON output schemas to ensure the AI response can be parsed.</li>
 *   <li>Injecting specific formatting instructions (XML tags) to support CLI syntax highlighting for Linux entities.</li>
...
public class LinuxPromptServiceImpl implements PromptService {
     ...
     * {@inheritDoc}
     * <p>
     * <strong>Linux Specifics:</strong> This method mandates specific XML tags ({@code <path>},
     * {@code <user>}, {@code <pid>}) in the 'example_output' field. These tags are parsed by
     * the CLI frontend to apply specific colors (e.g., Cyan for paths, Yellow for PIDs),
     ...
     * {@inheritDoc}
     * <p>
     * Sets the persona to "expert Linux System Administrator" to ensure the answers are technically
     * accurate regarding POSIX standards and system behavior.
     * </p>
     */
    @Override
    ...
    private String validateAndGetTopic(String moduleKey) {
        if (moduleKey == null) {
            throw new IllegalArgumentException("Invalid Linux module key: null");
        }
        String topic = MODULE_TOPICS.get(moduleKey);
        if (topic == null) {
            throw new IllegalArgumentException("Invalid Linux module key: " + moduleKey);
        }
        return topic;
    }
